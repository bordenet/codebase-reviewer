# PHASE II META-PROMPT - For Perplexity.ai Review

Please review and make improvements to the following LLM prompt which will be **embedded inside the Phase II tools** and used for **self-evolution and regeneration** when the tools detect they've become obsolete. This meta-prompt is the "DNA" of the Phase II tools - it gets re-emitted with learnings from previous generations when the codebase changes significantly enough that the tools can no longer keep up.

**Context**: This prompt is baked into the Go tools generated by Phase I. When the tools detect obsolescence (e.g., 30% of files changed, new languages detected, coverage drops below threshold), they emit an enhanced version of this prompt with learnings from their execution history. A human then sends this enhanced prompt to an LLM to generate the next generation of tools.
...
**Please provide**:
1. Critique of the meta-prompt's self-evolution design
2. Suggestions for improving the obsolescence detection strategy
3. Recommendations for better metrics tracking and learning capture
4. Evaluation of the balance between automation and human oversight
