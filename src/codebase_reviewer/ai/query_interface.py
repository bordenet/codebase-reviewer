        ...
        return [
            {
                ...
                'filter': lambda issue: 'SQL' in issue.get('rule_id', '').upper() or 'injection' in issue.get('description', '').lower(),
                'description': 'SQL injection vulnerabilities',
            },
            {
                'pattern': r'(?:show|find|list|get)\s+(?:me\s+)?(?:all\s+)?xss',
                'filter': lambda issue: 'XSS' in issue.get('rule_id', '').upper() or 'cross-site' in issue.get('description', '').lower(),
                'description': 'XSS vulnerabilities',
            },
            {
                'pattern': r'(?:show|find|list|get)\s+(?:me\s+)?(?:all\s+)?(?:hardcoded\s+)?secrets?',
                'filter': lambda issue: 'SECRET' in issue.get('rule_id', '').upper() or 'hardcoded' in issue.get('description', '').lower(),
            ...
            },
            {
                ...
                'filter': lambda issue: 'TODO' in issue.get('rule_id', '').upper() or 'todo' in issue.get('description', '').lower(),
                'description': 'TODO comments',
            },
            {
                'pattern': r'(?:show|find|list|get)\s+(?:me\s+)?(?:all\s+)?(?:missing\s+)?(?:doc|documentation)',
                'filter': lambda issue: 'DOC' in issue.get('rule_id', '').upper() or 'docstring' in issue.get('description', '').lower(),
                'description': 'Documentation issues',
            },
            {
                'pattern': r'(?:show|find|list|get)\s+(?:me\s+)?(?:all\s+)?complexity\s+issues',
                'filter': lambda issue: 'COMPLEX' in issue.get('rule_id', '').upper() or 'complexity' in issue.get('description', '').lower(),
                'description': 'Complexity issues',
            },
            {
                'pattern': r'(?:show|find|list|get)\s+(?:me\s+)?(?:all\s+)?test\s+issues',
                'filter': lambda issue: 'TEST' in issue.get('rule_id', '').upper() or 'test' in issue.get('description', '').lower(),
