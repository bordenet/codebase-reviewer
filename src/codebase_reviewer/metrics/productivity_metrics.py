...
class ProductivityTracker:
    ...
    def _collect_metrics(self, start: datetime, end: datetime, author: Optional[str]) -> ProductivityMetrics:
        ...
        try:
            # Get commit count
            cmd = ['git', 'log', '--oneline', f'--since={start.isoformat()}', f'--until={end.isoformat()}']
            if author:
            ...
            # Get files changed
            cmd = ['git', 'log', '--name-only', '--pretty=format:', f'--since={start.isoformat()}', f'--until={end.isoformat()}']
            if author:
                cmd.extend(['--author', author])
            ...
            cmd = ['git', 'log', '--numstat', '--pretty=format:', f'--since={start.isoformat()}', f'--until={end.isoformat()}']
            if author:
                cmd.extend(['--author', author])
