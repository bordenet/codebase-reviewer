...
class JSONExporter:
    ...
    def to_dict(self, analysis: CodeAnalysis) -> Dict[str, Any]:
        ...
        return {
            ...
            "dependencies": [self._dependency_to_dict(d) for d in analysis.dependencies] if analysis.dependencies else [],
            "complexity_metrics": analysis.complexity_metrics if analysis.complexity_metrics else {},
            "quality_issues": [self._issue_to_dict(i) for i in analysis.quality_issues] if analysis.quality_issues else [],
            "summary": {
                ...
                "critical_issues": len([i for i in analysis.quality_issues if i.severity.value == "critical"]) if analysis.quality_issues else 0,
                "high_issues": len([i for i in analysis.quality_issues if i.severity.value == "high"]) if analysis.quality_issues else 0,
            }
        }
